    ...
    <div class="card">
        <div class="carbook-summary">
            <div class="carbook-summary-box mb-4">
                ...
                <p><span class="icon"><i class="fa fa-calendar"></i></span>
                    <?php echo date('d M Y', strtotime($rentalCarsData['startDate'])); ?></p>
                <p><span class="icon"><i class="fa fa-map-marker"></i></span>
                    <?php echo @$rentalCarsData['pickLocation']; ?>
                </p>
            </div>
            <div class="carbook-summary-box mb-4">
                <h3>{{trans('lang.vehicle_type')}}</h3>
                <p><span class="icon"><img
                            src="<?php echo @$rentalCarsData['rentalVehicleType']['rental_vehicle_icon']; ?>" width="30"
                            height="30"></span><?php echo @$rentalCarsData['rentalVehicleType']['name']; ?></p>
                <p><span class="icon"><i class="fa fa-list-alt"></i></span>
                    <?php echo @$rentalCarsData['rentalVehicleType']['description']; ?></p>
            </div>
            <div class="carbook-summary-box mb-4">
                <h3>{{trans('lang.rental_package')}}</h3>
                <p><span class="icon"><i class="fa fa-clock-o"></i></span>
                    <?php echo @$rentalCarsData['rentalPackageModel']['name']; ?></p>
                <p><span class="icon"><i class="fa fa-list-alt"></i></span>
                    <?php echo @$rentalCarsData['rentalPackageModel']['description']; ?></p>
            </div>
...
<div class="col-md-4 pt-3 carbook-detail-right">
    <div class="siddhi-cart-item overflow-hidden bg-white sticky_sidebar" id="cart_list">
            ...
            <div class="carbook-payment-option">
                ...
                <div class="payselect-option">
                    <select name="Payment" id="payment">
                        ...
                        <option value="cash on delivery" style="display: none;" id="cod_box">{{trans('lang.cod')}}
                        </option>
                        <option value="razorpay" style="display: none;" id="razorpay_box">{{trans('lang.razor_pay')}}
                        </option>
                        ...
                        <option value="payfast" style="display: none;" id="payfast_box">{{trans('lang.pay_fast')}}
                        </option>
                        <option value="paystack" style="display: none;" id="paystack_box">{{trans('lang.pay_stack')}}
                        </option>
                        <option value="flutterwave" style="display: none;" id="flutterWave_box">
                            {{trans('lang.flutter_wave')}}</option>
                        <option value="mercadopago" style="display: none;" id="mercadopago_box">
                            {{trans('lang.mercadopago')}}</option>
                        <option value="xendit" style="display: none;" id="xendit_box">{{trans('lang.xendit')}}</option>
                        <option value="midtrans" style="display: none;" id="midtrans_box">{{trans('lang.midtrans')}}
                        </option>
                        <option value="orangepay" style="display: none;" id="orangepay_box">{{trans('lang.orangepay')}}
                        </option>
...
$total_price = $total_price - $discount;
                ...
                <span class="float-right text-dark"><?php echo "- "; ?><span class="currency-symbol-left"></span><?php if (@$rentalCarsData['coupon']['discount_amount'] && @$rentalCarsData['coupon']['discountType']) {
...
if (@$rentalCarsData['taxValue']) { ?>
    ...
    foreach ($rentalCarsData['taxValue'] as $val) {
        ...
        if ($val['type'] == 'fix') { ?>
                (
                <span
                    class="currency-symbol-left"></span><?php            echo number_format($val['tax'], $decimal_degits); ?><span
                    class="currency-symbol-right"></span></span>
                <?php            $tax = $val['tax']; ?>
                )
                <?php        } else {
            $tax = ($val['tax'] * $total_price) / 100; ?>
                ...
                <span class="float-right text-dark"><?php        echo "+ "; ?>
                    <span class="currency-symbol-left"></span><?php        echo number_format($tax, $decimal_degits); ?><span
                        class="currency-symbol-right"></span></span>
                <?php
...
$remainder = fmod($total_amount, 1000);
if ($remainder >= 500) {
    $total_amount = ceil($total_amount / 1000) * 1000;
} else {
    $total_amount = floor($total_amount / 1000) * 1000;
}
            ?>
            <hr>
