...
<script type="text/javascript">
        ...
        await vendorDetailsRef.get().then(async function (vendorSnapshots) {
            ...
            var specialOfferForHour = [];
             if (vendorDetails.hasOwnProperty("vendorCost")) {
                                ...
                                if(specialOfferVendor[i]['timeslot'].length > 0){
                                    for(j=0; j< specialOfferVendor[i]['timeslot'].length; j++){
                                        if(currentTime >= specialOfferVendor[i]['timeslot'][j]['from'] && currentTime <= specialOfferVendor[i]['timeslot'][j]['to']){
                                            ...
                                            if(specialOfferVendor[i]['timeslot'][j]['discount_type'] == 'dinein'){
                                                if(specialOfferVendor[i]['timeslot'][j]['type'] == 'percentage'){
                                                specialOfferDiscount += parseFloat(parseFloat(specialOfferVendor[i]['timeslot'][j]['discount'] * vendorDetails.vendorCost) / 100);
                                                }else{
                                                    specialOfferDiscount += parseFloat(specialOfferVendor[i]['timeslot'][j]['discount']);
                                                }
                                            }
                    ...
                    if (currencyAtRight) {
                        $("#cost_for_two").append("Cost for two - " + vendorDetails.vendorCost + "" + currentCurrency + " (Approx) ");
                        if(specialOfferDiscount > 0 && vendorDetails.vendorCost != ''){
                        ...
                        $("#cost_for_two").append("Cost for two - " + currentCurrency + "" + vendorDetails.vendorCost + " (Approx) ");
                        if(specialOfferDiscount > 0 && vendorDetails.vendorCost != ''){
    ...
    function showAllMenu() {
        var menuPhotoCount = 0;
        var menuCardPhotos = '';
        Array.from(vendorMenuPhotosOuter).forEach(vendorMenuPhotos => {
            menuPhotoCount++;
            menuCardPhotos = menuCardPhotos + '<div class="col-md-3"><span class="image-item" id="photo_menu' + menuPhotoCount + '"><span class="remove-btn" data-id="' + menuPhotoCount + '" data-img="' + vendorMenuPhotos + '"><img width="100px" id="" height="auto" src="' + vendorMenuPhotos + '" onerror="this.onerror=null;this.src=\'' + placeholderImageSrc + '\'"></div>';
        })
        $("#vendor_menus").html(menuCardPhotos);
    }
</script>
